...
const AnimatedNavLink = ({ href, children, isActive, onClick }: { href: string; children: React.ReactNode; isActive?: boolean; onClick?: (e: React.MouseEvent) => void }) => {
    ...
    return (
        <a
            href={href}
            onClick={onClick}
            className={`group relative inline-block overflow-hidden h-5 flex items-center ${textSizeClass} cursor-pointer`}
        >
            <div className="flex flex-col transition-transform duration-400 ease-out transform group-hover:-translate-y-1/2">
                <span className={`${defaultTextColor} transition-colors duration-200`}>{children}</span>
                ...
                <nav className="hidden sm:flex items-center space-x-4 sm:space-x-6 text-sm">
                    {navLinksData.map((link) => (
                        <AnimatedNavLink
                            key={link.label}
                            href="#"
                            isActive={activeFilter === link.value}
                            onClick={(e) => {
                                e.preventDefault();
                ...
                <nav className="flex flex-col items-center space-y-4 text-base w-full">
                    {navLinksData.map((link) => (
                        <a
                            ...
                            className={`text-gray-300 hover:text-white transition-colors w-full text-center ${activeFilter === link.value ? 'text-white font-semibold' : ''}`}
                        >
                            {link.label}
                        </a>
                    ))}
